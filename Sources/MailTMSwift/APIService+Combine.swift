...
extension APIService {
    func get<T: Decodable>(endpoint: String,
                           ...
                           headers: [String: String] = [:]) -> AnyPublisher<T, MTError> {
...
struct MTAPIServiceRequestPublisher<T: Decodable, E: Encodable>: Publisher {
    ...
    func receive<S>(subscriber: S) where S: Subscriber, MTError == S.Failure, Output == S.Input {
        let subscription = MTAPIServiceRequestSubscription(subscriber: subscriber,
                                                           apiService: apiService,
                                                           method: method,
                                                           endpoint: endpoint,
                                                           authToken: authToken,
...
final class MTAPIServiceRequestSubscription<S: Subscriber, T: Decodable, E: Encodable>: Subscription where S.Failure == MTError, S.Input == T {
    ...
    func request(_ demand: Subscribers.Demand) {
        ...
        self.task = apiService.request(
            method: method,
            endpoint: endpoint,
            authToken: authToken,
            headers: headers,
            body: body) { (result: Result<T, MTError>) in
            switch result {
                case .failure(let error):
                    self.subscriber?.receive(completion: .failure(error))
                case .success(let data):
